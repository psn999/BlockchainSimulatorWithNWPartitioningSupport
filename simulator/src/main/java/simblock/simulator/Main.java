...
import static simblock.settings.SimulationConfiguration.INFINITE_TIME;
import static simblock.settings.SimulationConfiguration.NETWORK_PARTITIONING_EVENTS;
...
import static simblock.simulator.Timer.performPartioning;
 ...
 * @throws Exception 
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (getTask() != null) {
      ...
      //check and perfrom network partitioning
      long currentTime=getCurrentTime();
      for(NetworkPartioningEvent event:NETWORK_PARTITIONING_EVENTS) {
    	  if(event.getEventTimestamp()<=currentTime) {
    		  performPartioning(event);
    		  event.setEventTimestamp(INFINITE_TIME);
    	  }
      }
    }
    ...
    /*
    Log in format:
     ï¼œfork_information, block height, block IDï¼ž
  ...
  //TODOã€€ä»¥ä¸‹ã�®åˆ�æœŸç”Ÿæˆ�ã�¯ã‚·ãƒŠãƒªã‚ªã‚’èª­ã�¿è¾¼ã‚€ã‚ˆã�†ã�«ã�™ã‚‹äºˆå®š
  //ãƒŽãƒ¼ãƒ‰ã‚’å�‚åŠ ã�•ã�›ã‚‹ã‚¿ã‚¹ã‚¯ã‚’ä½œã‚‹(ãƒŽãƒ¼ãƒ‰ã�®å�‚åŠ ã�¨ï¼Œãƒªãƒ³ã‚¯ã�®è²¼ã‚Šå§‹ã‚�ã‚‹ã‚¿ã‚¹ã‚¯ã�¯åˆ†ã�‘ã‚‹)
  //ã‚·ãƒŠãƒªã‚ªãƒ•ã‚¡ã‚¤ãƒ«ã�§ä¸Šã�®å�‚åŠ ã‚¿ã‚¹ã‚¯ã‚’Timerå…¥ã‚Œã�¦ã�„ã��ï¼Ž
